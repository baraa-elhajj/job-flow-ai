...
class JobSearchScraper(BaseScraper):
    ...
    async def search(
        ...
        job_urls, job_titles = await self._extract_job_urls_and_titles(limit)

        res = []
        for i in range(len(job_titles)):
            item = {}
            item["url"] = job_urls[i]
            item["title"] = job_titles[i]
            res.append(item)
    ...
    #     self, keywords: Optional[str] = None, location: Optional[str] = None
    # ) -> str:
    ...
    async def _extract_job_urls_and_titles(self, limit: int) -> List[str]:
        """
        ...
        try:
            job_post = await self.page.locator(".job-search-card").all()

            for jp in job_post:
                title = jp.locator(".base-search-card__title").first
                link = jp.locator('a[href*="/jobs/view/"]').first
                ...
                try:
                    ...
                    if href and "/jobs/view/" in href:
                        ...
                        if clean_url not in seen_urls:
                            ...
                            job_titles.append(await title.inner_text())
